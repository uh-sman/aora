...
export const getAllPosts = async () => {
  try {
    const posts = await databases.listDocuments(databaseId, videoCollectionId, [Query.orderDesc("$createdAt"), Query.equal('useId', userCollectionId)]);
...
export const getBookmarkedPost = async () => {
  try {
    const posts = await databases.listDocuments(databaseId, videoCollectionId, [Query.orderDesc("$createdAt"), Query.limit(3)]);
    return posts.documents
  } catch (error: any | unknown) {
    throw new Error(error);
  }
...
export const createVideo = async (form: FormState) => {
  ...
  try {
   ...
   const newPosts = await databases.createDocument(databaseId, videoCollectionId, ID.unique(), {
      title: form.title,
      video: videoUrl,
